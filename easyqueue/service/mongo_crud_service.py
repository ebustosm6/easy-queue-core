...
class MongoCRUDService(BaseCRUDService):
    ...
    def __init__(self, repository: MongoRepository = None, base_object_class: Type = None):
    ...
    def __get_validated_element(cls, element: EQObject, allow_none: bool = False) -> Dict:
        ...
        if isinstance(element, EQObject):
    ...
    async def find(self, query: Dict) -> List[EQObject]:
        TypeValidator.raise_validation_element_type(
            element_name='query', element=query, type_class=dict, allow_none=False)
        raw_results = await self.repository.find(query=query)
        parsed_results = []
        for raw_result in raw_results:
            parsed_results.append(self.base_object_class.from_json(raw_result))
        return parsed_results

    async def find_one(self, query: Dict) -> EQObject:
        TypeValidator.raise_validation_element_type(
            element_name='query', element=query, type_class=dict, allow_none=False)
        raw_result = await self.repository.find_one(query=query)
        return self.base_object_class.from_json(raw_result)
